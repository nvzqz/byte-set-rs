    ...
    for &size in util::SIZES {
        ...
        group.bench_function(
            ...
            |b| {
                b.iter_batched_ref(
                    || {
                        black_box(IdentityHashSet::<u8>::rand_len(
                            size, &mut rng,
                        ))
                    },
                    |hash_set| {
        ...
        group.bench_function(
            ...
            |b| {
                b.iter_batched_ref(
                    || {
                        black_box(IdentityHashbrownSet::<u8>::rand_len(
                            size, &mut rng,
                        ))
                    },
                    |hash_set| {
        ...
        group.bench_function(
            ...
            |b| {
                b.iter_batched_ref(
                    ...
                    |fixed_bit_set| {
                        // The `.ones()` iterator does not have `.next_back()`.
                        black_box(fixed_bit_set.ones().last());
                    },
                    BatchSize::SmallInput,
                )
            },
